...
public class ChamadosDAO
{
    ...
    public static List<ListaGrid> MostraChamadosNaTelaStatus0()
    {
        ...
        using (SqlConnection com = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["chamado_suporteConnectionString"].ToString()))
        {
            try
            {
                string strQuery = @"SELECT * FROM [chamado_suporte].[dbo].[aberturaChamado] where statusChamado='0'order by id ASC";
                com.Open();
                SqlCommand commd = new SqlCommand(strQuery, com);
  ...
  FROM [chamado_suporte].[dbo].[aberturaChamado] where (statusChamado = 5 or statusChamado = 6 or statusChamado = 7) order by id ASC";
                com.Open();
                SqlCommand commd = new SqlCommand(strQuery, com);
                SqlDataReader dr = commd.ExecuteReader();
                while (dr.Read())
